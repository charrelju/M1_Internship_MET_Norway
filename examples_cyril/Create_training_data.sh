...
paths["AMSR2"] = "/lustre/storeB/project/copernicus/osisaf/data/reprocessed/ice/conc/v3p0/"
#
proj = {}
...
def extract_TOPAZ_data(date_task, ndays, variables = variables["TOPAZ"], paths = paths):
    ...
    if len(dataset) == ndays:
        for lt in range(0, ndays):
            ...
            if lt == 0:
                Data_output = Data_lt.copy()
            else:
                Data_output["time"] = np.concatenate((Data_output["time"], Data_lt["time"]), axis = 0)
                for var in variables:
                    Data_output[var] = np.concatenate((Data_output[var], Data_lt[var]), axis = 0)
        #
        for var in variables:
            idx_assign = np.logical_or(np.isnan(Data_output[var]) == True, Data_output[var] == -32767)
            Data_output[var][idx_assign == True] =  0
    else:
        print("ERROR. Some forecast dates are missing")
    #
    return(Data_output)
